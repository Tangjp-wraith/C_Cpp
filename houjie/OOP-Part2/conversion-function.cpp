...
class Fraction {
 public:
   ...
   * 把分数当成一个double类型，转换函数
   * 表示分数可以转换为double
   * 转换函数没有返回类型，因为在函数名上就说明了转换成什么类型
   * 转换函数不会改变类型
   */
  operator double() const { return (double)(m_num * 1.0 / m_den); }
...
class Fraction1 {
 public:
  // non-explicit-one-argument ctor (非显式的一个参数)
  // 这样的构造函数使得编译器可以调用non explicit one argument constructor
  // 将4转换为Fraction1
...
class Fraction3 {
 public:
  // explicit 为了解决2的问题 表示我这个构造函数只能用来作为构造函数
  // 不能用于类型转换
